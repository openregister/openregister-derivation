...
public class LocalAuthorityByTypeTransformer implements RegisterTransformer {
        ...
        newPartialEntities.forEach(newEntity -> {
            newEntity.getEntries().forEach(entry -> {
                ...
                if (stateMap.containsKey(laType)) {
                    List<String> currentLocalAuthorities = (List<String>) stateMap.get(laType).getRecord().getItem().getFields().get(LOCAL_AUTHORITIES);
                    if (!currentLocalAuthorities.contains(localAuthority)) {
                        findEntityByAuthority(localAuthority, state)
                                .ifPresent(pe -> {
                                    List<String> localAuthList = (List<String>) pe.getRecord().getItem().getFields().get(LOCAL_AUTHORITIES);
    ...
    private Optional<PartialEntity> findEntityByAuthority(String localAuthority, Set<PartialEntity> state) {
        return state.stream().filter(e -> ((List<String>) e.getRecord().getItem().getFields().get(LOCAL_AUTHORITIES)).contains(localAuthority)).findFirst();
    }
    ...
    private Entry createLocalAuthoritiesEntryAdding(String localAuthority, List<String> currentLocalAuthorities, String localAuthorityType, AtomicInteger entryNumber) {
        ...
        return getEntry(updatedLocalAuthorities, localAuthorityType, entryNumber);
    }
    ...
    private Entry getEntry(List<String> newLocalAuthorities, String localAuthorityType, AtomicInteger entryNumber) {
        ...
        Entry newLocalAuthoritiesEntry = new Entry(entryNumber.getAndIncrement(), Instant.now(), hashValue("zz"));
